...
class LoRATrainerPro:
    ...
    def generate_dataset(self) -> List[Dict[str, str]]:
        self.logger.info("Génération dataset...")
        
        cfg = self.training_config
        dataset = []

        # Wikipedia
        articles = self.wiki_scraper.get_random_articles(cfg.num_articles)
        for article in tqdm(articles, desc="Wikipedia"):
            data = self.wiki_scraper.get_article_content(article['title'])
            if data and data['length'] > 200:
                qa_pairs = self.wiki_qa_gen.generate_qa_pairs(
                    data['title'], data['content'], data['category'], max_pairs=cfg.qa_per_article
                )
                dataset.extend(qa_pairs * 3)

        # OASST1
        dialogues = self.dialogue_loader.get_next_batch(cfg.num_dialogues)
        dataset.extend(dialogues * 3)

        random.shuffle(dataset)
        self.logger.info(f"Dataset: {len(dataset)} exemples")
...
def main():
    ...
    print(f"  Augmentation: {training_config.use_augmentation}")
    ...
    # Boucle de 100 cycles d'entraînement
    for cycle in range(10):  # 100 cycles
        print(f"\n{'='*70}")

...
def load_hf_dataset(dataset_name="HuggingFaceH4/ultrachat_200k", split="train_sft", max_samples=None):
    """Charge un dataset depuis HuggingFace et extrait le texte."""
    print(f"\n📥 Chargement du dataset: {dataset_name}")
    
    with tqdm(total=1, desc="🔄 Téléchargement HF", colour='cyan') as pbar:
    ...
    # Extraction du texte
    texts = []
    samples_to_use = min(len(ds), max_samples) if max_samples else len(ds)
    
    with tqdm(total=samples_to_use, desc="📝 Extraction texte", colour='cyan') as pbar:
        for i, example in enumerate(ds):
            if max_samples and i >= max_samples:
                break
            
            # Extraction du texte selon la structure du dataset
    ...
    full_text = "\n\n".join(texts)
    text_size_mb = len(full_text) / 1e6
    
    print(f"✅ Texte extrait: {len(full_text):,} caractères ({text_size_mb:.2f} MB)")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="🚀 Tokenizer BPE avec HuggingFace Dataset",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Exemples d'utilisation:
  # Entraîner avec dataset HuggingFace (50k tokens vocab)
  python tokenizer_hf.py --train --vocab_size 50000 --save tokenizer_50k.bin
  
  # Avec dataset personnalisé et limite d'échantillons
  python tokenizer_hf.py --train --vocab_size 50000 --hf_dataset "openai/webtext" --max_samples 10000 --save tok_50k.bin
  
  # Avec checkpoints (recommandé pour gros vocab)
  python tokenizer_hf.py --train --vocab_size 50000 --checkpoint checkpoints/tok --save tokenizer_50k.bin
  
  # Encoder du texte
  python tokenizer_hf.py --use_tokenizer --load tokenizer_50k.bin --input "Bonjour le monde!"
        """
    )
    
    parser.add_argument("--dataset", type=str, default=None,
                        help="Dataset local (fichier texte) - optionnel si --hf_dataset est utilisé")
    parser.add_argument("--hf_dataset", type=str, default="HuggingFaceH4/ultrachat_200k",
                        help="Dataset HuggingFace (défaut: HuggingFaceH4/ultrachat_200k)")
    parser.add_argument("--hf_split", type=str, default="train_sft",
                        help="Split du dataset HF (défaut: train_sft)")
    parser.add_argument("--max_samples", type=int, default=None,
                        help="Nombre max d'échantillons à utiliser (None = tous)")
    parser.add_argument("--save", default="./tokenizer_50k.bin", type=valid_tokenizer_model,
                        help="Chemin de sauvegarde du tokenizer")
    parser.add_argument("--load", default="./tokenizer_50k.bin", type=valid_tokenizer_model,
    ...
    if args.train:
        print("\n" + "="*60)
        print("🎯 MODE ENTRAÎNEMENT - TOKENIZER 50K")
        print("="*60)
        
        # Chargement du dataset
        if args.dataset and os.path.exists(args.dataset):
            # Dataset local
            with tqdm(total=1, desc="📖 Chargement dataset local", colour='cyan') as pbar:
                with open(args.dataset, "r", encoding="utf-8") as f:
                    data = f.read()
                pbar.update(1)
        else:
            # Dataset HuggingFace (par défaut)
            if args.dataset and not os.path.exists(args.dataset):
                print(f"⚠️  Fichier '{args.dataset}' introuvable, utilisation du dataset HuggingFace à la place")
        ...
        print(f"\n📊 Dataset final: {len(data):,} caractères ({data_size_mb:.2f} MB)")
